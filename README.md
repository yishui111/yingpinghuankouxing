...
- **已内置优化**：同一形象视频二次合成直接命中预处理缓存；音频不超视频长度时不循环画面（大提速）；多任务自动 GPU 排队；结果封装零二次转码 + faststart 网页秒开；支持「极速 / 精修 / 极速对话 / 自定义」四档
...
- 🖥️ **Web 控制台**（端口 5000）：素材「服务器文件 / 本地上传」双来源、极速版/精修版/极速对话/自定义四档、上传进度条、实时进度、在线预览与下载，刷新页面不丢任务
- ⚡ **智能提速**：人脸检测 + VAE 编码结果按视频内容缓存，同素材二次合成跳过最耗时环节
- 🔄 **画面智能循环**：音频 ≤ 视频长度顺序播放；音频更长时自动「正向 + 反向回环」无缝续画面（任务详情会给出音视频时长与回环提示）
- 🎛️ **参数全开放**：batch_size、面部混合范围（jaw/neck/raw）、上半脸保留比例、边缘模糊、质感统一/噪点重注入等十余项
- 🔁 **任务管理**：后台异步任务自动 GPU 排队，支持查询进度 / 取消任务 / 清空任务记录 / 预处理缓存查看与一键清理，一键刷新模型释放显存（/api/reload）
...
- **Q：合成结果很慢？** A：选「极速版」或「极速对话」，或给同一形象视频用同一参数再合成（命中预处理缓存会快很多）；显存允许时调大 batch_size。
- **Q：缓存/输出占空间太大？** A：控制台任务列表卡片右上角有「清理缓存」（预处理缓存，可随时重建）和「清空记录」按钮；接口层对应 `POST /api/cache/clear` 与 `POST /api/tasks/clear`。
