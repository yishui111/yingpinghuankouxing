...
def run_inference(task_id, params):
    """Run MuseTalk inference in background thread."""
    try:
        ...
        if get_file_type(video_path) == "video":
            ...
            cmd = f"ffmpeg -y -v fatal -i {video_path} -start_number 0 {save_dir_full}/%08d.png"
            os.system(cmd)
        ...
        for i, res_frame in enumerate(res_frame_list):
            ...
            if texture_align and not fast_blend:
        ...
        cmd_combine = f"ffmpeg -y -v warning -i {audio_path} -i {temp_vid_path} {output_vid_name}"
...
def api_health():
    return jsonify({
        "status": "ok" if _model_cache["ready"] else "loading",
        "model_ready": _model_cache["ready"],
        "device": str(_model_cache["device"]),
        "gpu_available": torch.cuda.is_available(),
        "gpu_mem": f"{torch.cuda.memory_allocated() // 1024 // 1024}MB / {torch.cuda.get_device_properties(0).total_memory // 1024 // 1024}MB" if torch.cuda.is_available() else "N/A",
    })
...
def api_reload():
    ...
    try:
        ...
        cache_items = list(_model_cache.items())
...
def api_synthesize():
    ...
    with _tasks_lock:
        _tasks[task_id] = {
            ...
            "progress": 0,
            "message": "排队中...",
            "params": {k: v for k, v in params.items() if k not in ["video_path", "audio_path"]},
...
def api_files():
    ...
    Each entry includes a "src" tag to show where the file comes from.
    """
    files = {"videos": [], "audios": []}
    # 按展示顺序扫描：先 data 后 materials
    scan_dirs = [
        ("/app/data", "data"),
        ...
        try:
            for f in sorted(os.listdir(data_dir)):
                ...
                if ext in [".mp4", ".avi", ".mov", ".mkv", ".webm"]:
                    files["videos"].append({"name": f, "path": path, "src": src})
                elif ext in [".wav", ".mp3", ".flac", ".m4a", ".aac"]:
                    files["audios"].append({"name": f, "path": path, "src": src})
        except Exception as e:
